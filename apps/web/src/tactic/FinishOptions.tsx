...
import { getActiveFinishOptionsEvents, type FinishOption } from "./finish-options-data";
...
export function FinishOptions({ document, courtMode = "half", visibleAtTimeMs }: Props) {
  const events = useMemo(() => {
    return getActiveFinishOptionsEvents(document.events, visibleAtTimeMs).map((item) => item.event);
  ...
  events.forEach((event, eventIdx) => {
    ...
    if (fromSvg) {
      nodes.push(
        <g key={`finish-source-${event.from ?? eventIdx}`}>
          <circle
            cx={fromSvg[0]}
            cy={fromSvg[1]}
            r={5.5}
    ...
    event.options.forEach((option, idx) => {
      ...
      const isShot = option.kind === "shot" || option.kind === "tip";
      const isPrimary = option.priority === "primary" || isShot;
      const stroke = option.kind === "tip" ? "#fb923c" : isShot ? "#ff7043" : "#4dd0e1";
      const fill = isShot ? "rgba(255, 112, 67, 0.16)" : "rgba(77, 208, 225, 0.14)";

      nodes.push(
        <g key={`finish-option-${event.from ?? eventIdx}-${idx}`}>
          {fromSvg ? (
            <line
              x1={fromSvg[0]}
              y1={fromSvg[1]}
